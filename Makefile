...
# all the sources
#
SRCS =	main.c sim-fast.c sim-pipe.c sim-pipe-cache.c sim-pipe-withstall.c sim-safe.c sim-cache.c sim-profile.c \
...
# programs to build
#
PROGS = sim-fast$(EEXT) sim-pipe$(EEXT) sim-pipe-cache$(EEXT) sim-pipe-withstall$(EEXT) sim-safe$(EEXT) sim-eio$(EEXT) \
...
sim-pipe-cache$(EEXT):	sysprobe$(EEXT) sim-pipe-cache.$(OEXT) $(OBJS) libexo/libexo.$(LEXT)
	$(CC) -o sim-pipe-cache$(EEXT) $(CFLAGS) sim-pipe-cache.$(OEXT) $(OBJS) libexo/libexo.$(LEXT) $(MLIBS)
...
sim-pipe-cache.$(OEXT): host.h misc.h machine.h machine.def regs.h memory.h
sim-pipe-cache.$(OEXT): options.h stats.h eval.h loader.h syscall.h dlite.h sim.h
